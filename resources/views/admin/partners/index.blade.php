                ...
                <tr class="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-3">
                            <button type="button" onclick='openEditPartner(@json($partner))' class="text-slate-600 bg-slate-100 hover:bg-emerald-100 hover:text-emerald-700 font-medium text-sm transition-colors px-3 py-2 rounded-lg">Edit</button>
...
<div id="modal-add-partner" class="fixed inset-0 bg-slate-900/50 z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-auto overflow-hidden">
        ...
        <form method="POST" action="{{ route('admin.partners.store') }}" enctype="multipart/form-data" class="p-4 sm:p-6">
            @csrf
            ...
            <div style="display:flex;justify-content:flex-end;gap:.75rem">
                <button type="button" onclick="document.getElementById('modal-add-partner').style.display='none'" class="px-4 py-2 border border-slate-200 rounded-lg bg-white text-slate-500 hover:bg-slate-50 text-sm font-medium transition-colors">Batal</button>
...
{{-- Modal Edit Partner --}}
<div id="modal-edit-partner" class="fixed inset-0 bg-slate-900/50 z-50 hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-auto overflow-hidden">
        <div class="flex items-center justify-between p-4 sm:p-6 border-b border-slate-200">
        ...
        <form id="edit-partner-form" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6">
            @csrf @method('PUT')
            <div style="margin-bottom:1rem">
                <label style="display:block;font-size:.85rem;font-weight:600;color:#374151;margin-bottom:.5rem">Nama Mitra <span style="color:#ef4444">*</span></label>
            ...
            <div style="display:flex;justify-content:flex-end;gap:.75rem">
                <button type="button" onclick="document.getElementById('modal-edit-partner').style.display='none'" class="px-4 py-2 border border-slate-200 rounded-lg bg-white text-slate-500 hover:bg-slate-50 text-sm font-medium transition-colors">Batal</button>
