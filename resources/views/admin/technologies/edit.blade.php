...
@section('page-title', 'Edit Data Peralatan & Mesin')
...
<div class="max-w-6xl mx-auto pb-12">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('technologies.show', $technology->slug) }}" target="_blank" class="inline-flex items-center gap-1.5 text-xs font-bold text-blue-600 hover:text-blue-800 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-200">
            <span>👁️ Lihat Halaman Publik (Preview)</span>
        </a>
    </div>

    <form action="{{ route('admin.technologies.update', $technology) }}" method="POST" enctype="multipart/form-data" id="techEditForm">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                <div class="lg:col-span-8 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="tool_name" required value="{{ old('tool_name', $technology->tool_name) }}" class="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-900">
                        </div>

                        <div>
                            <label class="text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-1.5 block">Tipe &amp; Brand Resmi</label>
                            <input type="text" name="type_brand" value="{{ old('type_brand', $technology->type_brand) }}" class="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-900">
                        </div>

                        <div>
                            <label class="text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-1.5 block">Label Badge Card</label>
                            <input type="text" name="badge_text" value="{{ old('badge_text', $technology->badge_text ?? 'ALAT RESMI') }}" class="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold text-emerald-700">
                        </div>
                    </div>

                    <div>
                        <label class="text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-1.5 block">Deskripsi Singkat</label>
                        <textarea name="description" rows="4" class="w-full bg-slate-50/50 border border-slate-200 rounded-xl p-3 text-xs text-slate-800 leading-relaxed">{{ old('description', $technology->description) }}</textarea>
                    </div>
                </div>
                ...
                <div class="lg:col-span-4 bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs space-y-4">
                    ...
                    <div class="w-full aspect-[4/3] bg-slate-950 rounded-2xl overflow-hidden border border-slate-200/80 relative group shadow-xs">
                        <img id="img-preview" src="{{ $technology->image_url }}" alt="Preview" class="w-full h-full object-cover">
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-[11px] text-slate-400 mt-2 font-medium">Biarkan kosong jika tidak ingin mengganti foto saat ini.</p>
                    </div>
                    ...
                    <div class="border-t border-slate-100 pt-4 space-y-4">
                        <div>
                            ...
                            <input type="number" name="order_priority" value="{{ old('order_priority', $technology->order_priority ?? $technology->sort_order) }}" class="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-4 py-2 text-xs font-bold text-slate-900">
                        </div>

                        <label class="flex items-center gap-2 cursor-pointer pt-1 text-xs font-extrabold text-slate-800">
                            <input type="checkbox" name="is_active" value="1" {{ $technology->is_active ? 'checked' : '' }} class="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500">
            ...
            <div class="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="main_spec" value="{{ old('main_spec', $technology->main_spec) }}" class="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-900">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="pipe_target" value="{{ old('pipe_target', $technology->pipe_target) }}" class="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-900">
                    </div>

                    <div class="sm:col-span-2">
                        <label class="text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-1.5 block">Keunggulan Utama</label>
                        <input type="text" name="main_advantage" value="{{ old('main_advantage', $technology->main_advantage) }}" class="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-900">
                    </div>
                </div>
                ...
                <div class="border-t border-slate-100 pt-5 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200/80 space-y-2">
                            ...
                            <input type="text" name="feature_1_value" value="{{ old('feature_1_value', $technology->feature_1_value) }}" class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-800">
                        </div>

                        <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200/80 space-y-2">
                            <span class="text-[11px] font-bold text-slate-500">Fitur Highlight 2</span>
                            <input type="text" name="feature_2_value" value="{{ old('feature_2_value', $technology->feature_2_value) }}" class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-800">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div id="tab-safety" class="js-tab-content hidden space-y-6">
            <div class="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xs space-y-5">
                ...
                <div>
                    ...
                    <textarea name="safety_guarantee_text" rows="4" class="w-full bg-slate-50/50 border border-slate-200 rounded-xl p-3 text-xs text-slate-800 leading-relaxed">{{ old('safety_guarantee_text', $technology->safety_guarantee_text) }}</textarea>
                </div>
                ...
                <div class="border-t border-slate-100 pt-5 space-y-4">
                    ...
                    <div id="faq-repeater-container" class="space-y-3">
                        ...
                        <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200/80 space-y-3 faq-item">
                            ...
                            <input type="text" name="faqs[{{ $index }}][question]" value="{{ $faq['question'] ?? '' }}" placeholder="Pertanyaan FAQ..." class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-900">
                            <textarea name="faqs[{{ $index }}][answer]" rows="2" placeholder="Jawaban FAQ..." class="w-full bg-white border border-slate-200 rounded-xl p-3 text-xs text-slate-800">{{ $faq['answer'] ?? '' }}</textarea>
                        </div>
                        @empty
                        <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200/80 space-y-3 faq-item">
                            <div class="flex justify-between items-center">
                            ...
                            <input type="text" name="faqs[0][question]" placeholder="Pertanyaan FAQ..." class="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-900">
                            <textarea name="faqs[0][answer]" rows="2" placeholder="Jawaban FAQ..." class="w-full bg-white border border-slate-200 rounded-xl p-3 text-xs text-slate-800"></textarea>
                        </div>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="p-5 bg-slate-50 rounded-2xl border border-slate-200 space-y-2">
                    ...
                    <div class="p-4 bg-white rounded-xl border border-slate-200 shadow-2xs space-y-1">
                        <div class="text-[12px] text-slate-600 truncate flex items-center gap-1">
                            ...
                            <span id="serp-slug" class="text-slate-600 font-medium">{{ $technology->slug }}</span>
                        </div>
                        <h4 id="serp-title" class="text-base text-blue-700 font-bold hover:underline cursor-pointer line-clamp-1">
                            {{ $technology->meta_title ?? $technology->tool_name . ' - Teknologi Pelancar Pipa Tanpa Bongkar | Rootera Plumbing' }}
                        </h4>
                        <p id="serp-desc" class="text-xs text-slate-600 leading-relaxed line-clamp-2">
                            {{ $technology->meta_description ?? $technology->description ?? 'Deskripsi spesifikasi teknis peralatan yang akan tampil di halaman pencarian Google...' }}
                        </p>
                    </div>
                </div>
            </div>
...
@push('scripts')
<script>
let faqCount = {{ count(old('faqs', $technology->faqs ?? [1])) }};
...
function updateSerpPreview() {
    const titleVal = document.getElementById('meta_title_input').value || '{{ $technology->tool_name }}';
    const descVal = document.getElementById('meta_desc_input').value || '{{ $technology->description }}';
